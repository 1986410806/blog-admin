...
const TableList = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: val => (
        <div>
          <span key={val}>{val.views}</span> | <span key={val}>{val.likes}</span> | <span key={val}>{val.comments}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      title: '操作',
      render: (_, record) => [
        <Button type='primary' size='small' href={'/article/edit/' + record._id} key={record.id}><EditOutlined /> 编辑</Button>,
      ],
    },
  ];
  return (
